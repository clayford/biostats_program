...
We can load the data directly from a URL. This is useful if you keep your on a version control service such as GitHub. Below we read in the `rds` version of our data which is the native R format for storing an object.
...
`summary` is fantastic for a quick glance at your data.

```{r}
# call summary on Death_90D, Death_28D and P_0h
summary(d$P_0h)
```
...
The p-value is about 0.39. If the proportion of survivors and non-survivors with MI truly was equal, this sample result is not unexpected. There's about a 39% chance of getting two proportions as different as these (or more different) if there truly was no difference in the proportions.
...
mosaic::prop.test(HTN ~ Death_90D, data = d, success = 1)
```


```{r}
prop.test(x = c(204, 397), n = c(323, 821))
```
...
Let's replicate the correlation between phosphate level and the SOFA score:
...
A statistic calculated from a sample is just an estimate. Another sample will almost certainly yield a different estimate. A _confidence interval_ gives us some idea of how uncertain we are about an estimate. It provides a lower and upper bound for our estimate. Traditionally we calculate 95% confidence intervals. 
...
1. sample 30 obs from a Normal distribution with mean 25 and SD 3
...
The point is that 95% confidence intervals either do or do not contain the "true" value. The _process_ results in CIs capturing the value about 95% of the time.
...
The article also reports a significant difference in mean SOFA scores between survivors and non-survivors (Table 1) using a t-test. Let's replicate the result and get a 95% confidence interval

```{r}
t.test(SOFA_CRRT_0h ~ Death_90D, data = d)
```
...
The p-value of 0.02 associated with the Phosphate coefficient is for the null hypothesis test that the coefficient is equal to 0. We have evidence that Phosphate seems to have a _small effect_ on the APACHE II score. 
...
Let's replicate the linear regression of APACHE II scores on Age, Sex, CCI, MAP_0h, UO_2hrs and P_0h, also featured in Table 2.

```{r}
m2 <- lm(APA_II_CRRT_0h ~ Age + Sex + CCI + MAP_0h + UO_2hrs + P_0h, data = d)
summary(m2)
```
...
Earlier we compared the proportions of survivors and non-survivors with myocardial infarction (MI). Recall the proportions were 0.11 versus 0.09. That's a difference of 0.02. That's usually called _risk difference_. We could also compare their odds using odds ratios.
...
```{r}
lr2 <- glm(Death_90D ~ HTN, data = d, family = binomial)
summary(lr2)
exp(coef(lr2))
exp(confint(lr2))
```
