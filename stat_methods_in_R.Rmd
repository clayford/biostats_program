...
About 11% of survivors experienced MI versus 9% of non-survivors. How likely are we to observe a difference as big or bigger than 0.02 if there really is no difference in the entire population?
...
Linear models, or multiple regression, allow us to estimate means conditional on several predictors. For example, Table 1 tells us the overall mean APACHE II score is 27.2
...
Let's replicate the SOFA analysis using the `lm` function.
...
The Estimates column in the Coefficients section are the "betas". They are basically weighted sums in an additive formula. These values match what are in Table 2 (the multivariate column for the SOFA section).

The P_0H coefficient (or beta) is 0.10. That says "each additional unit of Phosphate produces an increase of 0.10 in the SOFA mean score, assuming all else held constant."
...
The SOFA scores range from 1 to 21. A model with conditional means that are systematically "off" by 5 or 10 points is perhaps not great.
...
Consider the following probabilities and their difference. The differences vary quite a bit, but the odds ratios are virtually constant.
...
An odds ratio of 9 by itself doesn't tell us about absolute risk. Increasing the risk of illness from 0.001 to 0.009 may not be as concerning as increasing the risk of illness from 0.07 to 0.40.
...
Earlier we estimated the odds of surviving beyond 90 days after CRRT is about 23% higher for those who experienced myocardial infarction versus those who did not. We can get this same estimate using logistic regression.
...
The CI is (0.54, 1.25). The CI on the MI odds ratio overlaps 1 by a wide margin in both directions. (Recall a ratio of 1 is the same as equal odds.) We're not sure what effect having a MI has on the probability of dying. 
...
Survival analysis is the analysis of data on _the time to occurrence of a specific event_. Example: time to death within 90 days of CRRT initiation. 
...
An important feature of survival analysis data is that it usually contains "incomplete" or _censored_ data. These are subjects that do not experience the event of interest. Example: some subjects (thankfully) survive beyond 90 days after CRRT. However we can make use of their data. To exclude them would _bias_ our probability estimates, which is a fancy way of saying it would make our estimates wrong. 
...
Now we can create the Kaplan-Meier survivor curves for the three groups. This plots the estimated probability of surviving over time. To do this we'll use the survival package that comes with R.

The `survfit` function creates survival curves using model notation similar to `lm` and `glm`. In this case we want to model time_to_death_90D as a function of p-change, the grouping variable we just created. The only difference is we need to create a _survival object_ for our response variable using the `Surv` function. It takes two arguments: the variable we're modeling (time_to_death_90D) and the condition that defines the event of interest (Death_90D=="Yes").
...
Is there a difference in the survival curves? We can use the _log-rank test_ to assess this by way of the `survdiff` function. The null hypothesis is no difference in survival curves. The small p-value provides against the null hypothesis.
...
Figure 3 in the paper shows pairwise comparisons between the curves. The results can be replicated by simply subsetting data to compare two groups at a time. For example, Group 1 versus Group 2:
...
_The fully adjusted model_ in this case is the 
