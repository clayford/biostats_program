...
Table 4, 90-day mortality section.

```{r}
d$p_change <- cut(d$Delta_P, breaks = c(-Inf, -1.3, 0, Inf), 
                  labels = c("Group 1", "Group 2", "Group 3"),
                  include.lowest = TRUE)
xtabs(~ P0h_3groups + p_change, data = d, subset = survivor == "Non-Survivor")
```

```{r}
library(survival)
Surv(d$time_to_death_90D, d$Death_90D=="Yes")
surv.group <- survfit(Surv(time_to_death_90D, Death_90D=="Yes") ~ p_change,
                      data = d)
plot(surv.group, lty = 1:3, xlab = "Days")
...
The paper states "a multivariable Cox model showed that hyperphosphatemia was significantly associated with increased 90-day (HR 1.05, 95% CI 1.02–1.08, P = 0.001) mortality."

HR means _hazard ratio_. 
